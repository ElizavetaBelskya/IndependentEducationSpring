            ...
            <c:forEach var="i" begin="0" end="${orders.size()}">
                ...
                <div class="col">
                    <div class="card" id="card-order">
                        <div class="card-body">
                            <h5 class="card-title">Subject: ${orders.get(i).subject}</h5>
                            <h3 class="card-text">Price: ${orders.get(i).price}</h3>
                            <p class="card-text">Description: ${orders.get(i).description}</p>
                            <p class="card-text">Format: ${orders.get(i).online}</p>
                            <p class="card-text">Minimal rating: ${orders.get(i).minRating}</p>
                            <input type="hidden" class="order-id" value="${orders.get(i).id}">
                            <input type="submit" class = "card-btn gradient-btn" value="Take it!"/>
                        </div>
                    </div>
                </div>
